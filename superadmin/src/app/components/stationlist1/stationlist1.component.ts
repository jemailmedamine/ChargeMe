...
@Component({
  selector: 'app-stationlist1',
  templateUrl: './stationlist1.component.html',
  styleUrls: ['./stationlist1.component.css']
})
export class Stationlist1Component implements OnInit {
  name=""
  ...
  ancienNameBorn=""
 // test=""
  constructor(private superadminservice: SuperadminService,private formBuilder: FormBuilder, private cookies: CookieService, private router: Router,private route: ActivatedRoute) 
  {
  ...
  async submitCreateStationForm(event: Event)  {
    ...
    if (this.isFormValid()) {
      var labelE = document.getElementById("alertexist");
      var res= await this.superadminservice.getstationByRN(this.createStation.registerNumber).toPromise();
    if(res && res.createdAt)
    {
      ...
      var res= await this.superadminservice.createStation(station).toPromise();
      if(res && res.createdAt)
      {
  ...
  async showTerminalSuccessPopup(event: Event){
    ...
    if (this.isFormValidBorn()) {
      var res= await this.superadminservice.getBornbySerial(this.createTerminal.serial).toPromise();
    if(res && res.createdAt)
    {
      if(labelE)
      {
      ...
      var res= await this.superadminservice.getBornbyNameAndidStation(this.idStation,this.createTerminal.name).toPromise();
      if(res && res.createdAt)
      {
        if(labeln)
        {
      ...
      var res2= await this.superadminservice.createBorn(born).toPromise();
      if(res2 && res2.createdAt)
      {
        ...
        for (const conn of this.listConCheK) {
         ...
         //console.log("res2.id idddd :::: "+res2.id)
          ...
          var res3= await this.superadminservice.createBornConnecteur(bornconn).toPromise()
        }
        }
      location.reload();
      }
  ...
  async getstation() {
    if(!this.idowner)
    {
    var res = await this.superadminservice.getAllStation(this.page,this.confirmationAdmin).subscribe((station) => {
      this.allstation = station;
    })
  }else{
    var res = await this.superadminservice.getByStationOwner(this.idowner,this.page,this.confirmationAdmin).subscribe((station) => {
      this.allstation = station;
    })
  }
    // var zz=await this.superadminservice.getConnecteursChecked().subscribe((conn) => {
    //   this.listConDejaCheK = conn;
    //   })
  }
 async openBorneEditPopup(born:any) {
  ...
  var resp= await this.superadminservice.getBornbyId(born.id).toPromise();
  if(resp)
  {
    this.born=resp
  ...
  var s1=0
  var s2=0
  var res4= await this.superadminservice.getAllConnecteurs().toPromise();
  if(res4)
  {
    this.listCon=res4
    s1=1
  ...
  this.listConCheK=[]
  var list= await this.superadminservice.getConnecteursChecked(born.id).toPromise();
     if(list)
     {
...
async showTerminalSuccessPopupEdit(event: Event){
  ...
  if (this.isFormValidBorn()) {
    ...
    var res= await this.superadminservice.getBornbySerial(inter).toPromise();
  if(res && res.createdAt && this.born.serialNumber!=this.ancienSerialNbr)
  {
    ...
    var res= await this.superadminservice.getBornbyNameAndidStation(this.idStation,inter2).toPromise();
...
console.clear()
var variable= await this.superadminservice.UpdateBorn(this.born).toPromise();
      ...
      for (let i = 0; i < listInexs.length; i++) {
        ...
        var vall:any
        vall=listInexs[i].id
        var ress= await this.superadminservice.deleteBornConn(this.idBorn,vall).toPromise();
      // if(ress=="ok" )
      // location.reload();
      }
        this.showTerminalEditPopup = false;
...
if(!this.idowner)
{
  ...
  async nextB() {
    if (this.pageb < (Number(this.allborn.nbr) - 1)) {
      ...
      var res = await this.superadminservice.getAllBorn(this.pageb,this.idStation).subscribe((born) => {
        this.allborn = born;
      })
    }
  }
  ...
  async PreviousMod() {
    if (this.pagemod > 0) {
      ...
      var res = await this.superadminservice.getAllModerateurbystation(this.pagemod,this.idStation).subscribe((mod) => {
 ...
 async submitEditStationForm(event: Event) {
    ...
    if (this.isFormValid())
    {
      ...
      {
      var res= await this.superadminservice.getstationByRN(this.stationEdit.registreNumber).toPromise();
    if(res && res.createdAt && this.stationEdit.registreNumber!=this.anciRN)
    {
      ...
      var res= await this.superadminservice.UpdateStation(this.stationEdit).toPromise();
      if(res && res[0] === 1)
      {
        location.reload();
      }
  ...
  async deleteStation() {
    var res= await this.superadminservice.deleteStation(this.idStation).toPromise();
    if(res=="ok" )
    location.reload();
  ...
  openBornePopup(idStation:any): void {
    this.listCon=[]
    this.idStation=idStation
    var res4 =  this.superadminservice.getAllConnecteurs().subscribe((con) => {
      this.listCon = con;
    })
  ...
  async openDetailPopup(station: any) {
    ...
    if (this.selectedStationDetails && this.selectedStationDetails.stationName === station.station?.name) {
      ...
      {
        var res1 =  this.superadminservice.getAdminById(station.station?.AdminId).subscribe((admin) => {
      ...
      var res2 =  this.superadminservice.getAllBorn(this.pageb,station.station?.id).subscribe((born) => {
        this.allborn = born;
      })
      var res3 =  this.superadminservice.getAllModerateurbystation(this.pagemod,station.station?.id).subscribe((mod) => {
        this.allmoderateur = mod;
      })
      if(station.station?.ProprietaireId)
      {
        var res4 =  this.superadminservice.getOwnerById(station.station?.ProprietaireId).subscribe((owner) => {
  ...
  async deleteSuccess() {
    var res= await this.superadminservice.deleteBorn(this.idBorn).toPromise();
    if(res=="ok" )
    location.reload();
...
async filrage() {
  this.page=0
  var res = await this.superadminservice.getAllStationF(this.page,this.confirmationAdmin,this.name).subscribe((station) => {
    this.allstation = station;
  })
}
}